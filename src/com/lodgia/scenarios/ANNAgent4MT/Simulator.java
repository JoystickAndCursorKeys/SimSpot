...
import com.lodgia.genesys.neuralnet.AF_SpikingThreshholdSigma;
...
public class Simulator implements InterfaceEvaluationFunction {
	...
	int agentSize;
	double initialAgentX, initialAgentY,initialAgentA=0.5;
	PingSensor png;
	RandSensor rnd;
	
	
	public Simulator(TypedProperties _config,
			InterfaceWorldGenericRenderer pRenderer, int pDebugLevel) throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		...
		//Af = new AF_Threshhold();
		//Af = new AF_Sigmoid();
		//Af = new AF_ThreshholdKV();
		
		String activationFunctionName = cfg.gets( "ACTIVATIONFUNCTION" );
		Af = //new AF_Threshhold();
		 (InterfaceActivationFunction) 
		 	Class.forName( activationFunctionName ).newInstance();
		...
		boolean agentCreated = AddNewAgentToWorld( w2d );
		
		if( !agentCreated ) {	
			System.err.println("Agent not created and/or added");
			
		}
		
	}
	
	public boolean CreateAgentBrain()
	{
		if( evalSingleAgentController == null ) {
	...
	public boolean AddNewAgentToWorld(World2d _w2d)
	{
		...
		_w2d.setCollideCallback(collider);
		...
		a.setInitialPosHeading((int) initialAgentX, (int) initialAgentY, (float) initialAgentA ); 
		/*
		 * ( (float) (
		 * member +
		 * iteration) )
		...
		*/
		...
		int sensorcount=a.getSensorCount();
		if( sensorcount != cfg.geti("SIM_INPUTS") ) {
			System.err.println( "SIM_INPUTS != sensorcount " );
			System.err.println( "sensorcount = " + sensorcount );
		}
	...
	public World2d CreateWorld() 
	{
		World2d myW2d;
		
		myW2d = new World2d(-350, -300, 350, 300, cfg.geti("SIM_XTILES"),
		...
		for (int i = 0; i < es.entities.size(); i++) {
			...
			if (e.isType(WorldBuilderMazeConverterLib.type_agent)) {
				initialAgentX = e.getX();
				initialAgentY = e.getY();
			}
		}
		...
		// if( (iteration / 5) % 2 == 0 ) {
		Simulator_WorldBuilder.buildWorld(myW2d, es, cfg, 0);
		myW2d.setGravity(Simulator_WorldBuilder.getGravity());
		agentSize = Simulator_WorldBuilder.getAgentSize();
		// }
		// else
		// {
		// Simulator_WorldBuilder2.buildWorld(myW2d, es, cfg, iteration );
		// }
	...
	public InterfaceEvaluationResult evaluateSingle(
			InterfaceGenericGenome genome, int mode, boolean firstMemberFlag, int member, int members,
		...
		if( w2d == null ) {
			System.err.println("World not created");
			result.score = 0.0;
			return result;	
		}
		...
		if ( firstMemberFlag && mode != InterfaceEvaluationFunction.MODE_REPLAY) {
		...
		try {
			boolean appendIsTrue = true;
			
			FileWriter fr = new FileWriter(cfg.gets("states.dataDir")
					+ "/population_" + iteration + ".txt", appendIsTrue);
			BufferedWriter br = new BufferedWriter(fr);
			PrintWriter out = new PrintWriter(br);

			out.println(result.score);
		...
		try {
			...
			FileWriter fr = new FileWriter(cfg.gets("states.dataDir")
					+ "/scores_" + iteration + ".txt", appendIsTrue);
			BufferedWriter br = new BufferedWriter(fr);
			PrintWriter out = new PrintWriter(br);

			out.println(member + "/" + result.score + "/" + a.score);
			out.close();
			br.close();
			fr.close();
		} catch (IOException e) {
	...
	public class agentmonitor extends PhysicalAgent {
		...
		public String dump(String string) {
			...
			dmp += "lasty="+lasty+";";
			dmp += "dist="+dist+";";
			dmp += "lastdeltaheading="+lastdeltaheading+";";
			dmp += "lastdeltaheadingsame="+lastdeltaheadingsame+";";
			dmp += "lastheading="+lastheading+";";
			dmp += "totalheading="+totalheading+";";
		...
		public void setController(InterfaceController evalSingleAgentController) {
			super.setController(evalSingleAgentController);
		...
		public void reset() {
			...
			this.stop();
			this.tmpcalculatecolissionstatus = false;
			score = 0.0;
	...
	public class hybridNeuralNet implements InterfaceController {
		...
		public void adaptBrainToDNA(InterfaceGenericGenome gg) {
			...
			for (int i = 0; i < nets; i++) {
				for (int j = 0; j < neuralBlocks[i].or.outputsarray.length; j++) {
					neuralBlocks[i].or.outputsarray[ j ] = 0.0;
					//System.out.println("Clear output " + i + "/" + j);

				}
				//l.debug( neuralBlocks[i].debugDump() );
			}
	...
	public class neuralBlock implements InterfaceController {
		...
		public double ScaleInput(double x) {
			double y;
		...
		public void setInputs(double pinputarray[]) {
			...
			for (int t = 0; t < inputs; t++) {
				...
				try {
					...
					n.clampTo( /*ScaleInput*/ ( pinputarray[t] ));
